         ...
         <form onSubmit={handleSubmit} className="space-y-8 pb-10">
            ...
            {hasExpense && (
               <div className="bg-[#fcf8f7] border border-[#e8ded8] rounded-[1.5rem] p-6 space-y-6 animate-in slide-in-from-top-4 shadow-sm">
                  ...
                  <div className="pt-2">
                      <label className="text-[10px] font-bold tracking-widest text-[#b83a0a] uppercase mb-2 block ml-1">Tu Alias o CBU <span className="text-[#a39a95] font-normal lowercase">(opcional)</span></label>
                      <input value={alias} onChange={e=>setAlias(e.target.value)} placeholder="Para que te devuelvan la plata" className="w-full bg-[#fcf8f7] border border-[#e8ded8]/80 shadow-inner rounded-[1rem] p-4 font-bold text-[#1f1a17] focus:ring-2 focus:ring-[#b83a0a]/30 outline-none placeholder:text-[#a39a95] placeholder:font-normal" />
                      <p className="text-[#a39a95] text-[10px] mt-3 ml-1">Solo necesario para que el resto te pueda transferir.</p>
                  </div>
               </div>
            )}
            ...
            <div className="pt-4">
                <button type="submit" disabled={loading || !name.trim() || hasExpense === null} className="w-full py-5 bg-[#b83a0a] text-white rounded-[1.25rem] text-[16px] font-heading font-bold shadow-[0_8px_30px_rgba(184,58,10,0.3)] flex justify-center items-center gap-2 hover:bg-[#8a2905] transition-all disabled:opacity-50 mb-4">
                    Sumarme al Asado <Grill size={20} className="text-white"/>
                </button>
